/*
 * Davin Chia
 * Implements 5 different functions to check for words
 * 
 */
...
public class Driver2 {
	...
	static HashTable index;	//to store misspelled words
	...
	public static void analyseFile(File file) throws IOException {
		...
		for (int i = 0; i < words.size(); i++) {
			...
			if (!hash.contains(word)) {
				add = new Word(current.element(), current.num());
				index.add(index.hash(current), add);
			}
		}
		index.sort();
		misspelled = index.getAll();
	...
	static void printMisspelled() {
		...
		char comm;
		Word word;
		for (int i = 0; i < words.size(); i++) {	//we go through the file containing normal words and compare them to misspelled ones
			word = words.get(i);
			//System.out.println("check: " + word);
			//System.out.println(index.contains(word.element()));
			//index.printAll();
		...
		while(run) {
			...
			writeCorrected(file);	//we write the corrected version in case user does not want to alter words; later alterations will overwrite this
				...
				if (com == 'q') {
					...
					printMisspelled();	//prompt user to correct misspelled words
					writeCorrected(file);	//print misspelled words to file
				}
				else if (com == 'f') {
					break;
				}
